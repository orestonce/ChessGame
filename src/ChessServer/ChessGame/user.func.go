...
func (this *GameUser) RpcRegister(req RegisterRequest) (resp RegisterResponse) {
	...
	this.gameCore.gUserLocker.LockByString(req.Username)
	defer this.gameCore.gUserLocker.UnlockByString(req.Username)
...
func (this *GameUser) RpcLogin(req LoginRequest) (resp LoginResponse) {
	this.gameCore.gUserLocker.LockByString(req.Username)
	defer this.gameCore.gUserLocker.UnlockByString(req.Password)
...
func (this *GameRoom) RpcChat(username string, req ChatRequest) (resp ChatResponse) {
	message := ChatMessage{
		TimeStr:  time.Now().In(this.mgr.core.local).Format("2006-01-02 15:04:05"),
		Username: username,
		Text:     req.Text,
	}
